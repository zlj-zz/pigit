...
class DiffViewer(Component):
    ...
    def _install_content(
        ...
        doc: DiffContent,
        *,
        tokens: list[_RenderLine] | None,
    ) -> None:
        """Atomically replace structure + tokens; always drop pending tokenize.

        Callers must finish parsing ``doc`` before this runs so a failed parse
        ...
        self._heatmap_colors = doc.heatmap_colors
        self._line_numbers = doc.line_numbers
        self._line_langs = doc.line_langs
    ...
    def _tokens_at(
        ...
        """Tokens for line ``idx``, or a plain-text fallback while tokenize is pending."""
        if idx < len(self._render_tokens):
            return self._render_tokens[idx]
    ...
    def _heatmap_at(self, idx: int) -> tuple[str, tuple[int, int, int]]:
        ...
        if idx < len(self._heatmap) and idx < len(self._heatmap_colors):
            return self._heatmap[idx], self._heatmap_colors[idx]
        return " ", THEME.fg_dim
    ...
    def _set_plain_content(self, lines: list[str]) -> None:
        ...
        doc = DiffContent.from_plain_lines(
            lines, language=lang, tokenizer=self._tokenizer
        )
        tokens = doc.pre_tokenize_plain(self._tokenizer)
        self._install_content(doc, tokens=tokens)
    ...
    def set_content(self, diff_lines: list[str]) -> None:
        ...
        """
        # Parse before mutating viewer state so a failed parse leaves the
        # previous content + tokens consistent.
        doc = DiffContent.from_diff_lines(
            diff_lines, word_diff=self._word_diff, tokenizer=self._tokenizer
        )
        self._file_history_mode = False
        self._file_history_cache.clear()
        self._install_content(doc, tokens=None)

        # Async: syntax highlighting (slow for large files).
        current_gen = self._tokenize_gen
        content = list(doc.lines)
        line_langs = list(doc.line_langs)
    ...
    _ranges_to_segments = staticmethod(DiffContent.ranges_to_segments)
    _pre_tokenize_with = staticmethod(DiffContent.pre_tokenize_with)
