...
class RepoCommandHandler:
    ...
    def mkbranch(self, args: "Namespace") -> None:
        ...
        if blockers:
            # Split blockers into fatal (cannot auto-resolve) and
            # recoverable (uncommitted changes — can stash).
            from pigit.git.managed_repos import BLOCKER_RECOVERABLE

            fatal = [b for b in blockers if b.kind != BLOCKER_RECOVERABLE]
            recoverable = [b for b in blockers if b.kind == BLOCKER_RECOVERABLE]

            # Always report fatal blockers.
            if fatal:
            ...
            if fatal:
                # If there are fatal blockers, still exit — but also list
                # recoverable ones so user sees the full picture.
                if recoverable:
                    self.console.echo(
                        "\nAlso has uncommitted changes (above). "
                        "Resolve fatal issues first."
                    )
                self.console.echo("\nFix the issues and retry.")
                raise SystemExit(1)

            # Only recoverable blockers — offer stash.
            ...
            if confirm("\nAuto-stash, create branch, then pop? [Y/n]: "):
                ...
                if stash_issues:
                    ...
                    for name, desc in stash_issues:
                        self.console.echo(f"  ! {name}: {desc}")
                return

            self.console.echo("\nAborted. Fix the issues and retry.")
            raise SystemExit(1)
        ...
        if blockers:
            ...
            if fatal:
                ...
                if has_missing_branch:
                    self.console.echo(
                        "\nFix the issues and retry, or use -c/--create "
                        "to create missing branches."
                    )
                else:
                    self.console.echo("\nFix the issues and retry.")
                raise SystemExit(1)
